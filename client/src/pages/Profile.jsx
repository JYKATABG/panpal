...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { data } = await axios.get(`http://localhost:5500/api/v1/users/me/recipes`);
                        ...
                        {recipes.map((recipe) => (
                            <Link to={`/recipes/${recipe._id}`} key={recipe._id}>
                                ...
                                >
                                    ...
                                    />
                                    <h3 className="text-lg font-semibold text-gray-700">{recipe.title}</h3>
                                    <p className="text-sm text-gray-500 line-clamp-2">{recipe.description}</p>
                                    <div className="mt-2 text-xs text-gray-400">
                                        Cook Time: {recipe.cookTime} min · Difficulty: {recipe.difficulty}
                                    </div>
                                </motion.div>
                            </Link>
                        ))}
